                                                            ...
                                                            <div class="text-left">
                                                                <div class="row w-100 m-0">
                                                                    ...
                                                                    <div class="col-4 text-center">
                                                                        <div class="p-2 mr-4">
                                                                            <span class="h6 d-block" style=" @if($item->valid == 1) text-decoration: line-through; @endif ">{{__('menu.Price')}}: {{round($item->pivot->price, 2)}} {{__('general.SR')}}</span>
                                                                            @if($item->pivot->offer_id)
                                                                                <span class="h6 @if($item->valid == 1) text-success @else text-danger @endif mt-2" style=" @if($item->valid == 0) text-decoration: line-through; @endif ">{{__('menu.Offer Price')}}: {{round($item->pivot->offer_price, 2)}} {{__('general.SR')}}</span>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                </div>
                                            ...
                                            <div class="h4 font-default text-bold">
                                                ...
                                                @if((session()->has('point_claim_value')) && isset($reorder))
                                                    ...
                                                    <h6 class="mt-1 mb-2"><b
                                                            ...
                                                            style="font-size: smaller;" id="total"> {{$order->total - session('point_claim_value')}} {{__('general.SR')}}</span>
                                                    </h6>
                                                @else
                                                    <h6 class="mt-1 mb-2"><b
                                                            class="inset-right-5 text-gray-light">{{__('general.Total')}}: </b> <span
                                                            style="font-size: smaller;" id="total"> @if(!isset($reorder)){{$order->total - $order->points_paid}}@else {{$order->total}} @endif {{__('general.SR')}}</span>
                                                    </h6>
                                                @endif

                                            </div>
                                            @if(isset($reorder))
                                            <a class="uk-button w-100" style="border-radius: 5px;" href="{{route('re.order',$order->id)}}"
